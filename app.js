...
mm.add("(min-width: 577px)", ()=>{
    ...
    const diagramas = gsap.utils.toArray(".diag:not(:first-child)");
    const diagAnimation = gsap.from(diagramas, { yPercent: 100, stagger: 1})
    
    
    ScrollTrigger.create({
        trigger: ".split-section-desktop",
        start: "top top",
        end: "bottom bottom",
        pin: ".right",
        scrub: 1,
        animation: diagAnimation,
        scrub: true
    })
...
mm.add("(max-width: 576px)", ()=>{
    ...
    const diagramasMobile = gsap.utils.toArray(".diag2")    
    // const diagramasMobileAnimation = gsap.from(diagramasMobile, {scale: 0, y: 20, opacity: 0, stagger: 1})
    // ScrollTrigger.create({
    //     trigger: ".split-section-mobile",
    //     start: "-100 top",
    //     end: "bottom bottom",
    //     animation: diagramasMobileAnimation,
    //     markers: true
    // })

    diagramasMobile.forEach((item) => {
        gsap.from(item, {
          opacity: 0,
          y: '+=100',
          stagger: 1,
